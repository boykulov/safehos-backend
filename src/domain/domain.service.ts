...
export class DomainService {
  ...
  async checkDomain(url: string, companyId: string, userId: string) {
    ...
    if (existing && existing.decision !== 'pending') {
      return { domain, decision: existing.decision, riskScore: existing.riskScore, flags: [], message: existing.reason, eventId: null, cached: true };
    }
    ...
    return { domain, decision: 'suspicious', riskScore: score, flags, eventId, fullUrl: url, message: score >= 70 ? 'Высокий риск — отправлен модератору' : 'Подозрительный домен — отправлен на проверку' };
  }
  ...
  private async createSuspiciousEvent(domain: string, url: string, score: number, flags: string[], companyId: string, userId: string): Promise<string> {
    const existing = await this.domainRepo.findOne({ where: { domain, companyId, decision: 'pending' } });
    if (existing) return existing.id;
    const event = new DomainDecision();
    event.domain = domain; event.companyId = companyId; event.decision = 'pending';
    event.riskScore = score; event.reason = `URL: ${url} | Flags: ${flags.join(', ')}`;
  ...
  async saveDomainDecision(domain: string, companyId: string, decision: string, riskScore: number, reason: string, decidedBy: string, isGlobal: boolean): Promise<DomainDecision> {
    ...
    if (existing) {
      existing.decision = decision; existing.reason = reason; existing.decidedBy = decidedBy;
  ...
  async getRecentDecisions(companyId: string) {
    const decisions = await this.domainRepo.find({
      where: [
        { companyId, decision: 'blocked' },
        { companyId, decision: 'approved' },
      ],
    ...
    const global = await this.domainRepo.find({
      where: [
        { isGlobal: true, decision: 'blocked' },
        { isGlobal: true, decision: 'approved' },
      ],
      order: { updatedAt: 'DESC' },
    });
    const all = [...decisions, ...global];
    return all.map(d => ({ domain: d.domain, decision: d.decision, updatedAt: d.updatedAt }));
  }
